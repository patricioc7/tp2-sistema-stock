...
const logProductSell = async (stockId, customerId, qty) => {
  const log = new HistoryLog('sell', undefined, stockId, qty, customerId)
  await addNewHistoryLog(log)
}

const logIncreaseStock = async (stockId, qty) => {
  const log = new HistoryLog('increase', undefined, stockId, qty, undefined)
  await addNewHistoryLog(log)
}

const logDecreaseStock = async (stockId, qty) => {
  const log = new HistoryLog('decrease', undefined, stockId, qty, undefined)
  await addNewHistoryLog(log)
}
